...
- **Maison la Fleur v24 QA fix batch** (implemented, awaiting user testing) — Second implementation pass over the QA findings in [MAISON_LEFLEUR_V23_QA_FIXES.md](./MAISON_LEFLEUR_V23_QA_FIXES.md). A prior pass (reflected in `builds/typebot-export-maison-lefleur-agent-v24-*.json`, the client's re-export) had already fixed items 1, 2, 3, 5, 6, 13 flow-side. This batch closes out the rest:
  - **#8** (occasion/design validator too strict) — added "gift"/"present" (+ translated equivalents) to the occasion term whitelist in `g_mlf_validate_occasion`; added an acknowledgement text bubble before both proactive-handoff `Jump` blocks (`g_mlf_proactive_handoff_jump_occasion`, `_design`) instead of a silent pivot.
  - **#12** — closed, no change. Hard budget filtering (already shipped) makes the overage case rare enough that the existing prose disclosure is sufficient; no dedicated Yes/No button block added.
  - **#25** — RU/UK `budget_flexible` wording swapped to QA's suggested phrasing ("Без ограничений" / "Не має значення"). Hebrew register-mixing not independently re-verified this pass.
  - **#7** (agent invents delivery/hours/payment/refund facts) — added an explicit guardrail paragraph to both AI Selection system prompts (main + addon) instructing the model to never state these as fact and to redirect to the team instead; updated the `ev_mlf_interruption` event's trigger list so direct questions about delivery cut-off/hours/payment/refund now hand off to a human instead of being excluded from handoff.
  - **#10** (checkout dead end) — `Checkout Card` group now asks "anything else?" after the thanks message via a new choice input; "Shop for more" jumps back to `Occasion Question`, "No thanks" falls through to a closing message.
  - **#14** (unlocalized 4-dot placeholder) — new `card_freetext_placeholder` translation key (3 dots, 5 languages), wired into the Show Card Options cards block's `freeTextInput.placeholder`.
  - **#17 / #18** — `ChatHeader.tsx`'s call-menu now formats phone numbers for display (`formatPhoneForDisplay`, extracted to `packages/embeds/js/src/utils/formatPhoneForDisplay.ts` with a unit test); Aventura's 954 area code confirmed correct by the user, no change needed there. WhatsApp link gets a prefilled message and UTM params (`?text=I%27d+like+help+with+&utm_source=chatagent&...`).
  - **#24** (Sympathy/Get Well still offers "Red and romantic"/"Tropical and lush") — new `Condition` block (`g_mlf_design_occasion_branch`) inserted between Occasion Question and Design Preference Question, comparing `{{occasion}}` against `{{localized_occ_sympathy}}` (language-agnostic since both are computed from the same dict in the same turn). Sympathy branch routes to a new `g_mlf_design_pref_sympathy` group with "Soft and calming"/"Peaceful and serene" swapped in for the two mismatched options; all other occasions fall through to the original group unchanged.
  - **#22** (non-deterministic recommendations) — `temperature: 0.2` set on both AI Selection open-router blocks (schema already supported it, was just unset).
  - **#9** (GuestBubble line-clamp) — `line-clamp-2`/`line-clamp-3` added to both `CardSnapshotsRow` and `CardSnapshotGuestBubble` in `GuestBubble.tsx`. Link-path rendering there was already correct (contrary to the original QA note) — no button/link change was needed.
  - **#15** (header truncation, no tooltip) — native `title` tooltips added to the truncating spans in `ChatHeader.tsx` (contact line + header name, which previously had no overflow handling at all).
  - **#16** (VIEW FULL not centered) — explicit `text-center` added alongside the existing `flex justify-center` in `CardsCaroussel.tsx`'s link-path button (and the equivalent in `GuestBubble.tsx`'s `SnapshotPathButton`), so wrapped text stays centered too.
  - **#19** (Escape doesn't close call dropdown) — added a `keydown` listener in `ChatHeader.tsx`.
  - **#20** (duplicate startChat POST) — root cause not conclusively pinned down from static review; added a non-reactive `hasDispatchedInitialStart` guard in `Bot.tsx` as a defense-in-depth fix against any Solid effect re-entrancy. Needs live network-tab verification to confirm the duplicate is actually gone.

  Flow-level changes live in `builds/maison-lefleur-agent-v24-fixed.json` (copy of the client's v24 export + edits above; validated against `typebotV6Schema` and passes structural checks — dangling edges, undeclared variables, orphaned groups). Needs re-import into the builder and a manual test pass before promoting to Shipped Features.

  **Deploy status:** code-level fixes (#9, #15, #16, #17, #19, #20) shipped to production 2026-08-03 via `.\deploy.ps1` — both images rebuilt, pushed, and live on the VPS (confirmed via image `Created` timestamp + HTTP check). Note: VPS's `docker-compose.yml` had drifted to `root:root` ownership, blocking the `scp` step — fixed via `sudo chown rakib:rakib` on the VPS before the copy succeeded; worth checking why that happened if it recurs. Flow-level fixes are **not** deployed by this process — they only take effect once `builds/maison-lefleur-agent-v24-fixed.json` is re-imported into the builder and republished.

  **Left untouched this session per explicit instruction:** #4 (cold start / no loading indicator), #21 (some answers arrive as two bubbles), #23 (mobile responsiveness unverified) — still unresolved, not investigated.

## Shipped Features

- **Live flow-execution animation in the preview graph (n8n-style)** — During a builder test run, the flow canvas animates the execution path in real time instead of only static-highlighting the current input block. New `onNewBubbleBlockDisplayed(blockId)` callback threaded through the embed chain (`ChatContainer.tsx` → `Bot.tsx` → the `typebot-standard` web component, registered in `packages/embeds/js/src/constants.ts`'s `defaultBotProps`) fires in sync with each bubble's real on-screen reveal (respecting typing-emulation delay). `GraphProvider` gains `executedBlockIds`/`executedEdgeIds` + `resetExecutionTrail()`; `apps/builder/src/features/preview/components/WebPreview.tsx` populates them from `onNewBubbleBlockDisplayed`/`onNewInputBlock`, resolving crossed `typebot.edges` by matching `edge.from.blockId` against whichever of the last bubble/input actually owns the edge (heuristic — exact ordering between the two isn't guaranteed since `onNewInputBlock` fires before its sibling bubbles animate). `BlockNode`/`GroupNode`/`Edge` render three states: active/executing (bold `orange-9` border + pulsing glow-ring via new `.typebot-active-node` keyframe in `custom.css`, background fill), completed (`green-8`/`green-3`, static), default. Active edges get a flowing dashed-stroke animation (`.typebot-edge-flow`) plus new `active-arrow`/`green-arrow` SVG markers in `Edges.tsx`. Trail resets on `onInit` (new run) and when the Preview drawer closes.
- **Collapsible blocks side panel** — New collapse/expand toggle for `BlocksSideBar.tsx` (`apps/builder/src/features/editor/components/`), independent of the pre-existing lock/hover-peek behavior. `isCollapsed` state persists to `localStorage` (`leftSidebarCollapsedStorageKey` in `features/editor/constants.ts`); collapsing slides the panel off-screen and disables hover-driven auto-peek (early-return in `handleMouseMove`), replaced by a persistent `ArrowRight01Icon` tab pinned at the canvas edge to re-expand it (restores prior lock state). New i18n keys added to `en.json` only — other locales not translated.
