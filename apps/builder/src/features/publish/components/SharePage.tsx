...
type ViewMode = "fullscreen" | "bubble";
          ...
          <div className="flex gap-4">
            <div className="flex flex-col gap-4 flex-1/2">
              <div className="flex items-center justify-between">
                ...
                >
                  <Tabs.List>
                    <Tabs.Tab value="fullscreen">Full screen</Tabs.Tab>
                    <Tabs.Tab value="bubble">Chat bubble</Tabs.Tab>
                  </Tabs.List>
                </Tabs.Root>
              </div>
              <div className="flex flex-col gap-4 p-4 rounded-lg border bg-gray-1">
                {viewMode === "fullscreen" ? (
                  <>
                    ...
                    env.NEXT_PUBLIC_VERCEL_VIEWER_PROJECT_NAME ? (
                      hasProPerks(workspace) ? (
                        ...
                        <UpgradeButton
                          limitReachedType={t(
                            "billing.limitMessage.customDomain",
                          )}
                  ...
                  <>
                    {typebot && env.NEXT_PUBLIC_VIEWER_URL?.[0] && (
                      <div className="flex items-center gap-2">
                        <p className="text-sm shrink-0 min-w-0 truncate">
                          {`${env.NEXT_PUBLIC_VIEWER_URL[0]}/${publicId}?view=bubble`}
                        </p>
                        <CopyButton
                          textToCopy={`${env.NEXT_PUBLIC_VIEWER_URL[0]}/${publicId}?view=bubble`}
                        />
                      </div>
                    )}
                    {typebot?.customDomain && (
                      <div className="flex items-center gap-2">
                        <p className="text-sm shrink-0 min-w-0 truncate">
                          https://{typebot.customDomain}?view=bubble
                        </p>
                        <CopyButton
                          textToCopy={`https://${typebot.customDomain}?view=bubble`}
                        />
                      </div>
                    )}
                  </>
