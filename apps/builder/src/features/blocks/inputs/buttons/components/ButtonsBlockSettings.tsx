...
export const ButtonsBlockSettings = ({ options, onOptionsChange }: Props) => {
  ...
  const updateFreeTextEnabled = (enabled: boolean) =>
    onOptionsChange({
      ...options,
      freeTextInput: { ...options?.freeTextInput, enabled },
    });
  const updateFreeTextPlaceholder = (placeholder: string) =>
    onOptionsChange({
      ...options,
      freeTextInput: {
        ...options?.freeTextInput,
        enabled: options?.freeTextInput?.enabled ?? false,
        placeholder: placeholder || undefined,
      },
    });
  const updateFreeTextVariable = (variable?: Variable) =>
    onOptionsChange({
      ...options,
      freeTextInput: {
        ...options?.freeTextInput,
  ...
  const updateButtonStyle = (buttonStyle: "filled" | "outline" | undefined) =>
    onOptionsChange({ ...options, buttonStyle });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Field.Root className="flex-row items-center">
        <Field.Label className="font-medium">Button style</Field.Label>
        <BasicSelect
      ...
      <Field.Container>
        <Field.Root className="flex-row items-center">
          <Switch
            ...
            onCheckedChange={updateFreeTextEnabled}
          />
          <Field.Label className="font-medium">
        ...
        {(options?.freeTextInput?.enabled ?? false) && (
          <>
            <Field.Root>
              <Field.Label>Placeholder text</Field.Label>
              <DebouncedTextInputWithVariablesButton
                defaultValue={options?.freeTextInput?.placeholder ?? ""}
                onValueChange={updateFreeTextPlaceholder}
              />
            </Field.Root>
            <Field.Root>
              <Field.Label>
              ...
              <VariablesCombobox
                ...
                onSelectVariable={updateFreeTextVariable}
              />
            </Field.Root>
          </>
        )}
