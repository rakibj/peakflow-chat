    ...
    setTypebot((typebot) =>
      produce(typebot, (typebot) => {
        ...
        if ("eventId" in edge.from) {
          ...
          } else if (edge.from.itemId === "__skip__") {
            addEdgeIdToSkipButton(typebot, newEdge.id, {
              groupIndex,
              blockIndex,
            });
          } else if (edge.from.itemId === "__card_clicked__") {
            addEdgeIdToCardClicked(typebot, newEdge.id, {
              groupIndex,
              blockIndex,
            });
          } else if (edge.from.itemId === "__free_text__") {
            addEdgeIdToFreeText(typebot, newEdge.id, {
              groupIndex,
              blockIndex,
            });
          } else {
...
const addEdgeIdToSkipButton = (
  ...
  const block = typebot.groups[groupIndex].blocks[
    blockIndex
  ] as unknown as CardsBlock;
  if (!block.options) (block as unknown as { options: object }).options = {};
  const opts = block.options as NonNullable<CardsBlock["options"]>;
  if (!opts.skipButton) opts.skipButton = { enabled: false };
  opts.skipButton.outgoingEdgeId = edgeId;
};

const addEdgeIdToCardClicked = (
  typebot: Draft<Typebot>,
  edgeId: string,
  { groupIndex, blockIndex }: BlockIndices,
) => {
  ...
  if (!block.options) (block as unknown as { options: object }).options = {};
  const opts = block.options as NonNullable<CardsBlock["options"]>;
  opts.cardClickedOutgoingEdgeId = edgeId;
};

const addEdgeIdToFreeText = (
  typebot: Draft<Typebot>,
  edgeId: string,
  { groupIndex, blockIndex }: BlockIndices,
) => {
  ...
  if (!block.options) (block as unknown as { options: object }).options = {};
  const opts = block.options as NonNullable<CardsBlock["options"]>;
  opts.freeTextOutgoingEdgeId = edgeId;
};
...
const resetOutgoingEdgeIdProp = ({
  ...
  if (hasItem) {
    if (
      edge.from.itemId === "__skip__" &&
      block.type === InputBlockType.CARDS
    ) {
      const cardsBlock = block as unknown as CardsBlock;
      if (cardsBlock.options?.skipButton)
        cardsBlock.options.skipButton.outgoingEdgeId = undefined;
      return;
    }
    if (
      edge.from.itemId === "__card_clicked__" &&
      block.type === InputBlockType.CARDS
    ) {
      const cardsBlock = block as unknown as CardsBlock;
      if (cardsBlock.options)
        cardsBlock.options.cardClickedOutgoingEdgeId = undefined;
      return;
    }
    if (
      edge.from.itemId === "__free_text__" &&
      block.type === InputBlockType.CARDS
    ) {
      const cardsBlock = block as unknown as CardsBlock;
      if (cardsBlock.options)
        cardsBlock.options.freeTextOutgoingEdgeId = undefined;
      return;
    }
...
const removeExistingEdge = (
  ...
  typebot.edges = typebot.edges.filter((existingEdge) => {
    ...
    if (newEdge.from.itemId)
      return (
        existingEdge.from.itemId !== newEdge.from.itemId ||
        existingEdge.from.blockId !== newEdge.from.blockId
      );
