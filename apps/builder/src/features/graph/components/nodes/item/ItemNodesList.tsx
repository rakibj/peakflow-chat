...
export const ItemNodesList = ({
  ...
  return (
    ...
    >
      ...
      {block.type === InputBlockType.CARDS && groupId && (
        <CardSelectedNode
          block={block as unknown as CardsBlock}
          groupId={groupId}
        />
      )}
      {block.type === InputBlockType.CARDS &&
        (block as unknown as CardsBlock).options?.freeTextInput?.enabled &&
        groupId && (
          <FreeTextNode
            block={block as unknown as CardsBlock}
            groupId={groupId}
      ...
      {block.type === InputBlockType.CARDS &&
        (block as unknown as CardsBlock).options?.skipButton?.enabled &&
        groupId && (
          <SkipButtonNode
            block={block as unknown as CardsBlock}
            groupId={groupId}
...
const CardSelectedNode = ({
  ...
  <div className="flex px-4 py-2 border rounded-md relative items-center bg-gray-1">
    <p>Card selected</p>
    <BlockSourceEndpoint
      source={{ blockId: block.id, itemId: "__card_clicked__" }}
      groupId={groupId}
      className="absolute right-[-49px]"
    />
  </div>
...
const FreeTextNode = ({
  ...
  <div className="flex px-4 py-2 border rounded-md relative items-center bg-gray-1">
    <p>Free text</p>
    <BlockSourceEndpoint
      source={{ blockId: block.id, itemId: "__free_text__" }}
      groupId={groupId}
      className="absolute right-[-49px]"
    />
  </div>
...
const SkipButtonNode = ({
  ...
  <div className="flex px-4 py-2 border rounded-md relative items-center bg-gray-1">
    <p>{block.options?.skipButton?.label || "Skip"}</p>
    <BlockSourceEndpoint
      source={{ blockId: block.id, itemId: "__skip__" }}
      groupId={groupId}
      className="absolute right-[-49px]"
    />
  </div>
...
const checkIfDefaultItemIsNeeded = (
  ...
  if (block.outgoingEdgeId || block.type === LogicBlockType.CONDITION)
    return true;
  if (block.type === InputBlockType.CARDS) return false;
  if (block.type === InputBlockType.CHOICE) {
    if (block.options?.freeTextInput?.enabled) return true;
