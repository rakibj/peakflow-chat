...
import { ChatHeaderForm } from "./ChatHeaderForm";
...
export const ChatThemeSettings = ({
  ...
  const updateHeader = (header: NonNullable<Theme["chat"]>["header"]) =>
    onChatThemeChange({ ...chatTheme, header });

  const updateContact = (contact: NonNullable<Theme["chat"]>["contact"]) =>
    onChatThemeChange({ ...chatTheme, contact });
