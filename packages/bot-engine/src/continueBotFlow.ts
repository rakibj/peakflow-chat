...
import { appendConversationHistory } from "./appendConversationHistory";
...
import {
  evaluateInterruptionEvent,
  findInterruptionEvent,
} from "./events/evaluateInterruptionEvent";
...
export const continueBotFlow = async (
  ...
  if (isInputBlock(block) && isInputMessage(reply)) {
    ...
    if (
      "content" in parsedReplyResult &&
      !(reply?.type === "text" && reply.metadata?.isSkip)
    )
      newSessionState = appendConversationHistory(newSessionState, {
        role: "human",
        content: parsedReplyResult.content,
        blockId: block.id,
      });
    isFreeTextReply =
      reply?.type === "text" &&
      (block.type === InputBlockType.TEXT ||
        Boolean(reply.metadata?.isFreeText));
  }
  ...
  let finalWalkStartingPoint: typeof walkStartingPoint = walkStartingPoint;
  let interruptionLog: LogInSession | undefined;
  if (isFreeTextReply) {
    const interruptionResult = await evaluateInterruptionEvent(newSessionState);
    interruptionLog = interruptionResult.log;
    const interruptionEvent = findInterruptionEvent(newSessionState);

    if (
      interruptionResult.mentionedLocation &&
      interruptionEvent?.options?.locationVariableId
    ) {
      const locationVariable =
        newSessionState.typebotsQueue[0].typebot.variables.find(
          byId(interruptionEvent.options.locationVariableId),
        );
      if (locationVariable) {
        const { updatedState, newSetVariableHistory } =
          updateVariablesInSession({
            state: newSessionState,
            currentBlockId: interruptionEvent.id,
            newVariables: [
              {
                ...locationVariable,
                value: interruptionResult.mentionedLocation,
              },
            ],
          });
        newSessionState = updatedState;
    ...
    if (
      interruptionResult.detectedLanguage &&
      interruptionEvent?.options?.languageVariableId
    ) {
      const languageVariable =
        newSessionState.typebotsQueue[0].typebot.variables.find(
          byId(interruptionEvent.options.languageVariableId),
        );
      if (languageVariable) {
        const { updatedState, newSetVariableHistory } =
          updateVariablesInSession({
            ...
            newVariables: [
              {
                ...languageVariable,
                value: interruptionResult.detectedLanguage,
              },
            ],
          });
        newSessionState = updatedState;
    ...
    if (
      interruptionResult.shouldInterrupt &&
      interruptionEvent?.outgoingEdgeId
    ) {
      const resumeBlockId =
        walkStartingPoint.type === "group"
          ? walkStartingPoint.group.blocks.at(0)?.id
          : resolveEdgeTargetBlockId(
              newSessionState,
              walkStartingPoint.nextEdge?.id,
            );
      newSessionState = {
        ...newSessionState,
        returnMark: resumeBlockId
          ? { status: "pending", blockId: resumeBlockId }
          : undefined,
      };
      newSessionState = appendConversationHistory(newSessionState, {
        role: "bot",
        content:
          "[The customer's request above was handed off to contact/handoff information. That request has now been addressed — continue the conversation normally from here.]",
        blockId: interruptionEvent.id,
      });
      finalWalkStartingPoint = {
        type: "nextEdge",
        nextEdge: {
          id: interruptionEvent.outgoingEdgeId,
  ...
  const executionResponse = await walkFlowForward(finalWalkStartingPoint, {
    version,
    state: newSessionState,
    setVariableHistory,
  ...
  if (
    ...
    return {
      ...
      logs: (interruptionLog ? [interruptionLog] : [])
  ...
  return {
    ...
    logs: (interruptionLog ? [interruptionLog] : []).concat(
...
const saveInputVarIfAny = ({
  ...
  if (block.options?.variableId) {
    const foundVariable = newState.typebotsQueue[0].typebot.variables.find(
  ...
  const freeTextVariableId =
    reply.metadata?.isFreeText &&
    (block.type === InputBlockType.CHOICE ||
      block.type === InputBlockType.CARDS)
      ? block.options?.freeTextInput?.variableId
      : undefined;
  if (freeTextVariableId) {
    const freeTextVar = newState.typebotsQueue[0].typebot.variables.find(
      (variable) => variable.id === freeTextVariableId,
    );
    if (freeTextVar) {
      const { updatedState } = updateVariablesInSession({
        newVariables: [
...
const resolveEdgeTargetBlockId = (
  state: SessionState,
  edgeId: string | undefined,
): string | undefined => {
  if (!edgeId) return undefined;
  const edge = state.typebotsQueue[0].typebot.edges.find(byId(edgeId));
  if (!edge) return undefined;
  if (edge.to.blockId) return edge.to.blockId;
  const group = state.typebotsQueue[0].typebot.groups.find(
    byId(edge.to.groupId),
  );
  return group?.blocks.at(0)?.id;
};
