...
export const validateAndParseInputMessage = (
  ...
  switch (block.type) {
    ...
    case InputBlockType.CHOICE: {
      ...
      const choiceReply = parseSingleChoiceReply(message.text, {
        replyId: message.metadata?.replyId,
        items: displayedItems,
      });
      if (choiceReply.status === "success") return choiceReply;
      if (block.options?.freeTextInput?.enabled && message.metadata?.isFreeText)
        return { status: "success", content: message.text };
      return choiceReply;
    }
    ...
    case InputBlockType.CARDS: {
      if (!message || message.type !== "text") return { status: "fail" };
      if (block.options?.skipButton?.enabled && message.metadata?.isSkip)
        return {
          status: "success",
          content: message.text,
          outgoingEdgeId: block.options.skipButton.outgoingEdgeId,
        };
      const cardsReply = parseCardsReply(message.text, {
        block,
        variables,
        sessionStore,
        replyId: message.metadata?.replyId,
      });
      if (cardsReply.status === "success")
        return {
          ...cardsReply,
          outgoingEdgeId:
            block.options?.cardClickedOutgoingEdgeId ??
            cardsReply.outgoingEdgeId,
        };
      if (block.options?.freeTextInput?.enabled && message.metadata?.isFreeText)
        return {
          status: "success",
          content: message.text,
          outgoingEdgeId: block.options?.freeTextOutgoingEdgeId,
        };
      return cardsReply;
    }
  }
