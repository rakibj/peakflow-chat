...
import { fetchOgPreview } from "./fetchOgPreview";
...
const executeGroup = async (
  ...
  for (const block of group.blocks) {
    ...
    if (isBubbleBlock(block)) {
      ...
      let message = parseBubbleBlock(block as BubbleBlockWithDefinedContent, {
        version,
      ...
      if (
        message.type === BubbleBlockType.LINK &&
        message.content?.showPreview &&
        message.content?.url
      ) {
        const previewData = await fetchOgPreview(
          message.content.url,
          message.content.previewFallbackUrl,
        );
        if (message.content.previewTitle)
          previewData.title = message.content.previewTitle;
        message = {
          ...message,
          content: { ...message.content, previewData },
        };
      }
      messages.push(message);
      newSessionState = appendConversationHistory(newSessionState, {
        role: "bot",
        content: extractPlainTextFromBubbleMessage(message),
        blockId: block.id,
      });
      if (
    ...
    if (isInputBlock(block)) {
      const input = await formatInputForChatResponse(block, {
        variables: newSessionState.typebotsQueue[0].typebot.variables,
      ...
      newSessionState = appendConversationHistory(newSessionState, {
        role: "bot",
        content: summarizePresentedInput(block, input),
        blockId: block.id,
      });
      return {
        messages,
