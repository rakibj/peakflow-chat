...
import { executeJsonJoin } from "./blocks/logic/jsonJoin/executeJsonJoin";
import { executeJsonRemap } from "./blocks/logic/jsonRemap/executeJsonRemap";
...
export const executeLogic = async ({
  ...
  switch (block.type) {
    ...
    case LogicBlockType.JSON_REMAP:
      return executeJsonRemap(block, { state, sessionStore });
    case LogicBlockType.JSON_JOIN:
      return executeJsonJoin(block, { state });
  }
};
